...
func combineLastCharacters(array: [String]) -> String {
    for word in nonsenseArray {
        let lastWord = word[word.index(before: word.endIndex)]
...
//func digitFinder (digit: String) -> Bool {
//
//}
//digitFinder(digit: "000")
...
var dirtyWordsArray = ["phooey", "darn", "drat", "blurgh", "jupiters", "argh", "fudge"]
var clernArray: [String] = []
func removeFourLetterWords (dirtyWords: [String]) -> [String] {
    for word in dirtyWordsArray {
        if word.count == 4 {
           clernArray.append(word)
        }
    }
    return clernArray
}
print(removeFourLetterWords(dirtyWords: dirtyWordsArray))
...
class MovieArchive {
    func filterByDirector(movieDictionary: Dictionary<String, String>, director: String) -> [String] {
        for movie in movies {
            if (movie.value.contains(director)) {
                directorMovie.append(movie.key)
            }
        }
        return directorMovie
    }
}
var movieDirector = MovieArchive()
print(movieDirector.filterByDirector(movieDictionary: movies, director: "Christopher Nolan"))
